...
import { XssHelper, XssObservation } from "../../helpers/xss.helper";
import {
...
test.describe("XSS & Input Validation", { tag: "@xss" }, () => {
  ...
  test("[TC-015] — Script injection in email field is blocked", async ({
    loginPage,
    logger,
    page,
  }) => {
    ...
    const results: Array<{
      payload: IXssPayload;
      obs: Awaited<ReturnType<typeof runXssEmailTest>>;
    }> = [];
  ...
  test("[TC-016] — HTML tag injection in email field is blocked", async ({
    loginPage,
    logger,
    page,
  }) => {
    ...
    for (const payload of htmlInjectionPayloads) {
      ...
      expect(await loginPage.isLoggedIn(), "HTML injection must not grant access").toBe(false);
    }
  ...
  test("[TC-017] — Event handler and javascript: URI injection is blocked", async ({
    loginPage,
    logger,
    page,
  }) => {
    ...
    for (const payload of eventHandlerPayloads) {
      ...
      expect(await loginPage.isLoggedIn(), "Event handler injection must not grant access").toBe(
        false,
      );
    }
  ...
  test("[TC-018] — Special characters are handled safely in email and password fields", async ({
    loginPage,
    logger,
    page,
  }) => {
    ...
    for (const payload of specialCharPayloads) {
      ...
      await xssHelper.attachListeners(page, BASE_HOST);
      await loginPage.login(XSS_DUMMY_EMAIL, payload.payload);
  ...
  test("[TC-019] — SQL-like injection does not grant unauthorised access", async ({
    loginPage,
    logger,
    page,
  }) => {
    ...
    const sqlPayloads = specialCharPayloads.filter((p) => p.id === "SC-002" || p.id === "SC-003");
    ...
    for (const payload of sqlPayloads) {
      ...
      logger.info(`[${payload.id}] isLoggedIn=false ✓, errorVisible=${errorShown}`);

      // Verify application responded with an error (not a blank page)
      const isResponded = errorShown || (await loginPage.isOnLoginPage());
      expect(
        isResponded,
  ...
  test("[TC-020] — Excessively long input (1 000 chars) is handled without crash", async ({
    loginPage,
    logger,
    page,
  }) => {
    ...
    expect(await loginPage.isLoggedIn(), "Long input must not grant access").toBe(false);
    expect(xssHelper.isXssExecuted(obs), "Long input must not execute XSS").toBe(false);
  ...
  test("[TC-021] — Excessively long input (10 000 chars) does not crash the server", async ({
    loginPage,
    logger,
    page,
  }) => {
  ...
  test("[TC-022] — Malformed and control character input is handled safely", async ({
    loginPage,
    logger,
    page,
  }) => {
  ...
  test("[TC-023] — Browser-side XSS verification — critical payload suite", async ({
    loginPage,
    logger,
    page,
  }) => {
    ...
    for (const payload of criticalPayloads) {
      ...
      logger.info(`[${payload.id}] UI: errorVisible=${errorShown}, onLoginPage=${onLoginPage}`);
      ...
      expect(obs.scriptElementFound, `[${payload.id}] No <script> element must appear in DOM`).toBe(
        false,
      );
      expect(
        obs.externalRequestMade,
    ...
    expect(executed.length, "All critical payloads must be blocked — none should execute").toBe(0);
  });
});
